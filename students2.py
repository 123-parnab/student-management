...
class student:
    def __init__(self,root):
        ...
        txt_search = Entry(detail_frame, width=20, textvariable=self.search_txt, font=("times new roman", 10, "bold"),
                           bd=5, relief=GROOVE)
    ...
    def add_students(self):
        if self.roll_no_var.get()=="" or self.name_var.get()=="":
            ...
            con=pymysql.connect(host="localhost",user="root",password="parnabmysql@123",database="stm2")
            cur=con.cursor()
            cur.execute("insert into students2 values(%s,%s,%s,%s,%s,%s,%s)",(self.roll_no_var.get(),
                                                                             self.name_var.get(),
                                                                             self.email_var.get(),
                                                                             self.gender_var.get(),
    ...
    def update_data(self):
        con = pymysql.connect(host="localhost", user="root", password="parnabmysql@123", database="stm2")
        cur = con.cursor()
        cur.execute("update students2 set name=%s,email=%s,gender=%s,contact=%s,dob=%s,address=%s where roll_no=%s",
                                                                         (self.name_var.get(),
                                                                          self.email_var.get(),
                                                                          self.gender_var.get(),
                                                                          self.contact_var.get(),
    ...
    def delete_data(self):
        con = pymysql.connect(host="localhost", user="root", password="parnabmysql@123", database="stm2")
        cur = con.cursor()
        cur.execute("delete from students2 where roll_no=%s",self.roll_no_var.get())
        con.commit()
        con.close()
        self.fetch_data()
        self.clear()
    ...
    def search_data(self):
        ...
        cur.execute("select * from students2 where "+str(self.search_by.get())+" LIKE '%"+str(self.search_txt.get())+"%'")
        rows=cur.fetchall()
        if len(rows) != 0:
            self.student_table.delete(*self.student_table.get_children())
            for row in rows:
